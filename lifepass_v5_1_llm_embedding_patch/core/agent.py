...
from .llm_assistant import generate_grounded_response
...
def build_agent_plan(profile: UserProfile, benefits: Iterable[Dict[str, Any]], question: str = "") -> Dict[str, Any]:
    ...
    rag_bundle = build_rag_context(evidence_query, benefits_list, safe_profile, top_k=5)
    docs = rag_bundle.get("hits", [])
    embedding_meta = rag_bundle.get("embedding", {})
    tool_trace.append(_tool(
        "embedding_rag_retriever",
        "ok",
        f"근거 chunk {len(docs)}개 검색 / provider={embedding_meta.get('provider', embedding_meta.get('provider_requested', 'unknown'))}",
    ))
    ...
    rule_summary = (
        f"총 {len(evaluations)}개 정책 판정, 가능 {len(eligible)}개, "
    ...
    llm_result = generate_grounded_response(
        ...
        rag_context=rag_bundle.get("context", ""),
        rule_summary=rule_summary,
        validation_warnings=warnings,
    )
    tool_trace.append(_tool(
        "bounded_llm_explainer",
        llm_result.get("mode", "template"),
        "LLM은 설명·요약·상담 문장 생성에만 사용하고 최종 판정은 rule_engine 결과를 따름",
    ))
    answer = llm_result.get("answer_markdown") or make_agent_answer(safe_profile, insights, plan, cliff_warning_count, docs, question)
    return {
        ...
        "evidence": docs,
        "rag_context": rag_bundle.get("context", ""),
        "llm_mode": llm_result.get("mode", "template"),
        "warnings": warnings,
    }
