...
def render_advanced_ai(profile: UserProfile) -> None:
    ...
    st.subheader("Embedding RAG policy retrieval")
    search_rows = search_policies(question, benefits, top_k=8)
    render_df(pd.DataFrame(search_rows), label="정책 의미 검색 결과", height=300)
    rag_status = embedding_rag_status()
    llm_info = llm_status()
    cemb1, cemb2, cemb3 = st.columns(3)
    cemb1.metric("Embedding provider", rag_status.get("provider_requested", "local_hash"))
    cemb2.metric("Embedding model", rag_status.get("model_requested", "local-hash-ko-384"))
    cemb3.metric("LLM mode", llm_info.get("provider", "template"))
    st.caption("임베딩 모델은 정책 문서·혜택 카탈로그에서 근거를 찾는 데 사용하고, LLM은 검색된 근거와 rule_engine 결과를 쉬운 상담 문장으로 바꾸는 데만 사용합니다. 최종 자격 판정은 여전히 deterministic rule_engine이 담당합니다.")

    st.subheader("LLM-grounded counseling answer")
    llm_question = st.text_area("LLM에게 맡길 상담 질문", question, height=90, key="llm_grounded_question")
    if st.button("Embedding RAG + LLM 상담 답변 생성", key="run_llm_rag_answer"):
        with trace_span("ui.llm_embedding_rag", question=llm_question[:80]):
            pack = answer_policy_question(llm_question, profile, benefits, top_k=6)
        st.session_state.last_llm_rag_pack = pack
    pack = st.session_state.get("last_llm_rag_pack")
    if pack:
        st.markdown(pack.get("answer_markdown", "해당 데이터 없음"))
        meta_cols = st.columns(4)
        meta_cols[0].metric("RAG hits", pack.get("retrieval", {}).get("count", 0))
        meta_cols[1].metric("LLM provider", pack.get("llm", {}).get("provider", "template"))
        meta_cols[2].metric("LLM mode", pack.get("llm", {}).get("mode", "template"))
        meta_cols[3].metric("월 환산효과", money(int(pack.get("monthly_support", 0))))
        if pack.get("llm", {}).get("warning"):
            st.warning(pack["llm"]["warning"])
        render_df(pd.DataFrame(pack.get("retrieval", {}).get("hits", [])), label="Embedding RAG 근거 chunk", height=340)
        with st.expander("RAG context / safety boundary", expanded=False):
            st.code(pack.get("retrieval", {}).get("context", ""), language="text")
            st.info(pack.get("safety_boundary", "LLM은 설명 보조에만 사용됩니다."))
    else:
        st.info("버튼을 누르면 현재 프로필과 정책 카탈로그를 기준으로 RAG 근거와 LLM 상담 답변을 생성합니다. API 키가 없으면 안전한 템플릿 모드로 동작합니다.")
